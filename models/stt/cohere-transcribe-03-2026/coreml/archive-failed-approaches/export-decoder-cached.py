...
class MaskedCachedDecoderWrapper(nn.Module):
    """Use DynamicCache with proper attention masking."""
    ...
    def forward(self, input_id, encoder_hidden_states, cache_k, cache_v, step, cross_attention_mask):
        """
        Use DynamicCache with masking approach.
        """
        batch_size = 1
        device = input_id.device
        dtype = encoder_hidden_states.dtype

        # Build self-attention cache WITHOUT masking - let attention mask handle it
        self_attention_cache = DynamicCache()
        cross_attention_cache = DynamicCache()

        for layer_idx in range(self.num_layers):
            # Self-attention cache - pass through as-is
            layer_k = cache_k[layer_idx].unsqueeze(0)  # (1, num_heads, max_seq_len, head_dim)
            layer_v = cache_v[layer_idx].unsqueeze(0)

            # Don't mask the cache - attention mask handles valid positions
        ...
        # Extract cache from FixedSizeCache
        ...
        for layer_idx in range(self.num_layers):
            ...
            # Ensure consistent shape (max_seq_len)
            ...
            if current_len < self.max_seq_len:
                ...
                # Keep the LAST max_seq_len positions (most recent tokens)
...
def export_decoder_cached(output_dir: Path, precision: str = "float16"):
    print("="*70)
    print("Cohere Decoder Export - DynamicCache with Masking")
    print("="*70)
