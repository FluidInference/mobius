#!/usr/bin/env python3
"""Test optimized decoder with pre-computed cross-attention K/V."""
...
print("="*70)
print("Test: Optimized Decoder with Pre-computed Cross-KV")
print("="*70)
...
cross_kv_projector = ct.models.MLModel("build/cohere_cross_kv_projector.mlpackage", compute_units=ct.ComputeUnit.CPU_AND_GPU)
decoder = ct.models.MLModel("build/cohere_decoder_optimized.mlpackage", compute_units=ct.ComputeUnit.CPU_AND_GPU)
print(f"   ✓ Loaded 3 models (encoder, cross-KV projector, optimized decoder)")
...
for sample_idx, sample in enumerate(samples):
    ...
    cross_kv_output = cross_kv_projector.predict({
    ...
    cross_k = None
    cross_v = None
    for key, value in cross_kv_output.items():
        if hasattr(value, 'shape') and len(value.shape) == 4:
            if 'k' in key.lower() or key == 'cross_k':
                cross_k = value
            else:
                cross_v = value
...
print(f"\n{'='*70}")
print("SUMMARY - OPTIMIZED DECODER")
print(f"{'='*70}")
print(f"Configuration: Optimized Decoder with Pre-computed Cross-KV")
...
if avg_wer < 10:
    ...
    print(f"\n⚠️  PARTIAL SUCCESS: {avg_wer:.2f}% WER - still has issues")
else:
    print(f"\n❌ FAILURE: {avg_wer:.2f}% WER - decoder still broken")
