...
def convert_num_channels(x: Tensor, num_channels: int) -> Tensor:
    # Note: num_channels is always a Python int (from self.encoder_dim[i])
    # and x.shape[-1] is a compile-time constant during tracing.
    # Using int() explicitly ensures Python-level comparison/arithmetic.
    cur_channels = int(x.shape[-1])
    if num_channels <= cur_channels:
        return x[..., :num_channels]
    else:
        pad_width = int(num_channels - cur_channels)
        return torch.constant_pad_nd(x, [0, pad_width], 0.0)
