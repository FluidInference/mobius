...
class SimpleDownsample(torch.nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        if torch.jit.is_tracing():
            assert (
                pad == 0
            ), f"pad should be zero for CoreML export. Given {pad}"
        else:
...
class SimpleUpsample(torch.nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        # Use repeat_interleave to avoid reshape with traced shape values
        return src.repeat_interleave(upsample, dim=0)
...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def forward(
        ...
        q = q.unflatten(-1, (num_heads, query_head_dim))
        p = p.unflatten(-1, (num_heads, pos_head_dim))
        k = k.unflatten(-1, (num_heads, query_head_dim))
        ...
        if use_pos_scores:
            ...
            pos_emb = pos_emb.unflatten(-1, (num_heads, pos_head_dim)).permute(
                2, 0, 3, 1
            )
            ...
            if torch.jit.is_tracing():
                # Avoid aten::Int by using Python constants and unflatten.
                # pos_scores: (num_heads, 1, time1, 2*time1-1) [batch_size=1 for export]
                nh = self.num_heads  # Python int constant
                time1 = pos_scores.shape[2]  # constant-folded during trace
                rows = torch.arange(start=time1 - 1, end=-1, step=-1)
                cols = torch.arange(time1)
                ps_flat = pos_scores.flatten(0, 2)  # (nh*1*time1, 2*time1-1)
                # repeat rows nh times (batch_size=1), not ps_flat.shape[0] times
                rows_rep = rows.repeat(nh).unsqueeze(-1)  # (nh*time1, 1)
                indexes = rows_rep + cols  # (nh*time1, time1)
                ps_flat = torch.gather(ps_flat, dim=1, index=indexes)
                # (nh*time1, time1) -> (nh, time1, time1) -> (nh, 1, time1, time1)
                pos_scores = ps_flat.unflatten(0, (nh, -1)).unsqueeze(1)
            else:
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        x = x.unflatten(-1, (num_heads, -1)).permute(2, 1, 0, 3)
