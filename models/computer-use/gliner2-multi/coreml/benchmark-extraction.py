...
def main():
    ...
    precision = runtime.selected_precision(schema) if args.adaptive_w8 else args.precision
    compute_units = "all" if args.adaptive_w8 and precision == "fp32" else args.units
    started = time.perf_counter()
    runtime.extract(text, schema)
    cold_first_request_ms = (time.perf_counter() - started) * 1000
