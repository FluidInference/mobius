...
FIXTURES = [
    ...
    ("María trabaja en Madrid para Telefónica.", ["persona", "organización", "ciudad"]),
    ("Jean Dupont travaille à Paris pour Airbus.", ["personne", "entreprise", "ville"]),
    ("张伟在北京大学工作。", ["人物", "机构", "地点"]),
    ("Angela Merkel besuchte Berlin.", ["Person", "Stadt"]),
]
...
def main():
    ...
    features = ct.models.MLModel(
        str(folder / f"gliner2_multi_extraction_features_{suffix}.mlpackage"), compute_units=ct.ComputeUnit.CPU_ONLY
    )
    scorer = ct.models.MLModel(
        str(folder / f"gliner2_multi_extraction_scorer_{suffix}.mlpackage"), compute_units=ct.ComputeUnit.CPU_ONLY
    )
    cases = []
    ...
    report = {
        ...
        "selected_manifest": "five fixed English, Spanish, French, Chinese and German entity fixtures",
        "cases": cases,
