...
def fixtures():
    ...
    return [
        ...
        (
            "es_entities",
            "María trabaja en Madrid para Telefónica.",
            Schema().entities(["persona", "organización", "ciudad"]),
        ),
        (
            "fr_entities",
            "Jean Dupont travaille à Paris pour Airbus.",
            Schema().entities(["personne", "entreprise", "ville"]),
        ),
        ("zh_entities", "张伟在北京大学工作。", Schema().entities(["人物", "机构", "地点"])),
        ("de_entities", "Angela Merkel besuchte Berlin.", Schema().entities(["Person", "Stadt"])),
    ]
...
def main():
    ...
    result = {
        ...
        "selected_manifest": "fifteen fixed real-text schema fixtures, not a Decision Index score",
