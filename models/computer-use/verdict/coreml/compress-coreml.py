"""Palettize a verified Verdict Core ML bucket and record the package size."""
...
def main() -> None:
    ...
    parser.add_argument("--packages", default="build")
    ...
    if args.max_candidates != 25:
        raise ValueError("Verdict's released head has 25 candidate slots")
    source = root / f"verdict_fp16_L{args.length}_candidates25.mlpackage"
    suffix = f"lut{args.bits}_{args.mode}_{args.granularity}"
    output = root / f"verdict_{suffix}_L{args.length}_candidates25.mlpackage"
