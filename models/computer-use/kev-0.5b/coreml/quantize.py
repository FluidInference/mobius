...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, help="explicit FP16 source package")
    parser.add_argument("--output", type=Path, help="explicit destination package")
    args = parser.parse_args()
    stem = f"kev_0_5b_fp16_L{args.length}_options{args.max_options}"
    source = args.source or args.build_dir / f"{stem}.mlpackage"
    target = args.output or args.build_dir / f"{stem.replace('fp16', args.precision)}.mlpackage"
    if target.exists():
