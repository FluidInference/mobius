...
Usage:
    uv run python verify/test_coreml_e2e_fp16.py \
        ...
        --output-wav build/wavs/e2e_coreml_fp16.wav
"""
...
def _load_mlmodels(compute_units: str, flow_precision: str = "fp16", hift_precision: str = "fp16"):
    cu = {
    ...
    # fp16v2: same tag on the mlpackage filename but different build dir
    flow_dir_tag = flow_precision
    flow_file_tag = "fp16" if flow_precision.startswith("fp16") else flow_precision
    flow_mlp = (BUILD / f"flow-{flow_dir_tag}-n250" / f"Flow-N250-{flow_file_tag}.mlpackage")
    hift_mlp = (BUILD / f"hift-{hift_precision}-t500" / f"HiFT-T500-{hift_precision}.mlpackage")
...
def main() -> None:
    ...
    print(f"      N_prompt={N_prompt}  max_new_tokens={max_new}  (FLOW_N_TOTAL={FLOW_N_TOTAL})")

    speech_w = _speech_embed_table(model_dir)

    print(f"[2/5] Loading CoreML models "
          f"(compute_units={args.compute_units}, flow={args.flow_precision}, hift={args.hift_precision})…")
    pre_model, dec_model, flow_model, hift_model = _load_mlmodels(
        args.compute_units, args.flow_precision, args.hift_precision)
    ...
    if top_id in STOP_IDS:
        print(f"      [early stop] first token {top_id} is a stop token")
    else:
    ...
    for step in range(1, max_new):
        ...
        logits = torch.from_numpy(dec_out["speech_logits"])[0, 0]
