# Full TTS Pipeline Test V7
# Uses V6 non-streaming prefill + V2 decode
import numpy as np
...
def test_full_pipeline():
    print("=" * 60)
    print("Full TTS Pipeline Test V7 (V6 Non-Streaming Prefill)")
    print("=" * 60)
    ...
    t0 = time.time()
    lm_prefill = ct.models.MLModel("qwen3_tts_lm_prefill_v6.mlpackage", compute_units=ct.ComputeUnit.CPU_ONLY)
    ...
    print(f"   TTS embeddings shapes: BOS={tts_bos_embed.shape}, PAD={tts_pad_embed.shape}, EOS={tts_eos_embed.shape}")
    ...
    print(f"   Expected prefill length: {3 + 4 + 1 + 1 + text_len + 1 + 1} = {text_len + 11}")

    # === LM Generation ===
    print("\n3. LM Generation (CoreML V6+V2)...")
    ...
    generated_tokens = []
    current_kv = kv_cache
    prefill_len = kv_cache.shape[3]
    position = prefill_len  # Start from after prefill
    ...
    while len(generated_tokens) < MAX_CODEC_TOKENS:
        result = lm_decode.predict({
            ...
            "trailing_text_embed": tts_pad_embed,  # Constant in non-streaming mode
            "kv_cache": current_kv,
    ...
    output_file = "test_full_pipeline_v7_output.wav"
