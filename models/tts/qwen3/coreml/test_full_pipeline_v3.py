# Full TTS Pipeline Test V3
# Uses V3 prefill with attention masking
import numpy as np
...
def test_full_pipeline():
    print("=" * 60)
    print("Full TTS Pipeline Test V3")
    print("=" * 60)
    ...
    t0 = time.time()
    lm_prefill = ct.models.MLModel("qwen3_tts_lm_prefill_v3.mlpackage")
    lm_decode = ct.models.MLModel("qwen3_tts_lm_decode.mlpackage")
    code_predictor = ct.models.MLModel("qwen3_tts_code_predictor_v3.mlpackage")
    ...
    # Truncate KV cache to actual_len (remove padding positions)
    actual_len = text_len + 2  # lang + text + bos
    ...
    generated_tokens = []
    current_kv = kv_cache
    # Position starts at actual_len (text_len + 2)
    position = actual_len
    ...
    output_file = "test_full_pipeline_v3_output.wav"
