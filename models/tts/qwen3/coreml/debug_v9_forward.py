# Debug V9's transformer forward vs official
import torch
import numpy as np
...
with torch.no_grad():
    ...
    actual_len = text_length + 11  # tensor([25])

print(f"V9 hidden shape: {v9_hidden.shape}")
print(f"Actual len: {actual_len.item()}")

# Build V9's mask
print("\n=== V9 mask construction ===")
with torch.no_grad():
    # V9's position embeddings
    ...
    print(f"Mask at [0,0,24,:30]: {combined_mask[0,0,24,:30].tolist()[:30]}")

# Run V9's first layer manually
print("\n=== V9 first layer ===")
with torch.no_grad():
    layer = wrapper.layers[0]
    hidden_states = v9_hidden

    residual = hidden_states
    hidden_states_normed = layer.input_layernorm(hidden_states)

    seq_len_full = FIXED_SEQ_LEN
    ...
    q = q.view(batch_size, seq_len_full, num_heads, head_dim).transpose(1, 2)
    k = k.view(batch_size, seq_len_full, num_kv_heads, head_dim).transpose(1, 2)
    v = v.view(batch_size, seq_len_full, num_kv_heads, head_dim).transpose(1, 2)
    ...
    k_expanded = k_expanded.reshape(batch_size, num_heads, seq_len_full, head_dim)
    v_expanded = v.unsqueeze(2).expand(-1, -1, n_rep, -1, -1)
    v_expanded = v_expanded.reshape(batch_size, num_heads, seq_len_full, head_dim)

    # Attention
    attn_weights = torch.matmul(q, k_expanded.transpose(-2, -1)) / (head_dim ** 0.5)
    print(f"Attn weights shape: {attn_weights.shape}")
    print(f"Attn weights at [0,0,24,:30]: {attn_weights[0,0,24,:30].tolist()[:10]}")

    attn_weights = attn_weights + combined_mask
    print(f"Attn weights after mask at [0,0,24,:30]: {attn_weights[0,0,24,:30].tolist()[:10]}")

    attn_weights = torch.softmax(attn_weights, dim=-1)
    print(f"Attn weights after softmax at [0,0,24,:30]: {attn_weights[0,0,24,:30].tolist()[:10]}")

    attn_output = torch.matmul(attn_weights, v_expanded)
    attn_output = attn_output.transpose(1, 2).reshape(batch_size, seq_len_full, -1)
    ...
    hidden_after_layer = residual + hidden_mlp

    print(f"\nV9 hidden after layer 0 at position 24: {hidden_after_layer[0, 24, :5].tolist()}")

# Run official model's first layer
print("\n=== Official first layer ===")
with torch.no_grad():
    # Extract actual sequence for official model
    ...
    # Get rotary for actual length
    cos_off, sin_off = talker.model.rotary_emb(v9_inputs, position_ids_official)

    layer = talker.model.layers[0]
    residual = v9_inputs
    hidden_states_normed = layer.input_layernorm(v9_inputs)

    q = layer.self_attn.q_proj(hidden_states_normed)
    k = layer.self_attn.k_proj(hidden_states_normed)
    v = layer.self_attn.v_proj(hidden_states_normed)

    q = q.view(1, seq_len_actual, num_heads, head_dim).transpose(1, 2)
    k = k.view(1, seq_len_actual, num_kv_heads, head_dim).transpose(1, 2)
    v = v.view(1, seq_len_actual, num_kv_heads, head_dim).transpose(1, 2)

    # Apply rotary
    q, k = apply_rotary_pos_emb_simple(q, k, cos_off, sin_off)

    # GQA expansion
    k_expanded = k.unsqueeze(2).expand(-1, -1, n_rep, -1, -1)
    k_expanded = k_expanded.reshape(1, num_heads, seq_len_actual, head_dim)
    v_expanded = v.unsqueeze(2).expand(-1, -1, n_rep, -1, -1)
    v_expanded = v_expanded.reshape(1, num_heads, seq_len_actual, head_dim)

    # Attention with causal mask only
    attn_weights = torch.matmul(q, k_expanded.transpose(-2, -1)) / (head_dim ** 0.5)
    print(f"Official attn weights at [0,0,24,:10]: {attn_weights[0,0,24,:10].tolist()}")

    causal_only = torch.triu(
        torch.ones(seq_len_actual, seq_len_actual, dtype=torch.float32),
        diagonal=1
    )
    causal_only = causal_only.masked_fill(causal_only == 1, float("-inf"))
    causal_only = causal_only.unsqueeze(0).unsqueeze(0)

    attn_weights = attn_weights + causal_only
    print(f"Official attn weights after mask at [0,0,24,:10]: {attn_weights[0,0,24,:10].tolist()}")

    attn_weights = torch.softmax(attn_weights, dim=-1)
    print(f"Official attn weights after softmax at [0,0,24,:10]: {attn_weights[0,0,24,:10].tolist()}")

    attn_output = torch.matmul(attn_weights, v_expanded)
    attn_output = attn_output.transpose(1, 2).reshape(1, seq_len_actual, -1)
    attn_output = layer.self_attn.o_proj(attn_output)
    ...
    hidden_mlp = layer.post_attention_layernorm(hidden_after_attn)
    hidden_mlp = layer.mlp(hidden_mlp)
    hidden_after_layer_official = residual + hidden_mlp

    print(f"\nOfficial hidden after layer 0 at position 24: {hidden_after_layer_official[0, 24, :5].tolist()}")

# Compare
print("\n=== Comparison ===")
v9_pos24 = hidden_after_layer[0, 24, :]
official_pos24 = hidden_after_layer_official[0, 24, :]
diff = (v9_pos24 - official_pos24).abs().max().item()
print(f"Max diff at position 24: {diff}")
