# Qwen3-TTS LM → CoreML Conversion v8
# Non-streaming mode with CORRECT sequence:
# - Position 7: tts_bos + speaker (NOT tts_pad + speaker)
# - Text starts at position 8 (NOT 9)
# - Total length: text_len + 10
...
MAX_TEXT_LENGTH = 128
# CORRECT Fixed sequence layout:
# 0-2: role prefix (3)
# 3-6: tts_pad + codec_think tokens (4)
# 7: tts_bos + speaker (1)
# 8 to 8+MAX_TEXT_LENGTH-1: text + codec_pad (MAX_TEXT_LENGTH)
# 8+MAX_TEXT_LENGTH: tts_eos + codec_pad (1)
# 8+MAX_TEXT_LENGTH+1: tts_pad + codec_bos (1)
# Total: 3 + 4 + 1 + MAX_TEXT_LENGTH + 1 + 1 = MAX_TEXT_LENGTH + 10

FIXED_SEQ_LEN = MAX_TEXT_LENGTH + 10  # CORRECTED from 11 to 10
...
class TracablePrefillV8(nn.Module):
    """Non-streaming prefill with CORRECTED sequence layout."""
    ...
    def forward(self, role_ids: torch.Tensor, text_ids: torch.Tensor, text_length: torch.Tensor,
        ...
        # Position 7: tts_bos + Speaker (CORRECTED!)
        hidden_states[:, 7:8, :] = tts_bos_embed + speaker_embed.unsqueeze(1)

        # Position 8 to 8+MAX_TEXT_LENGTH-1: Text tokens + codec_pad (SHIFTED from 9!)
        ...
        # Scatter eos to position 8 + text_length (CORRECTED from 9!)
        eos_idx = (8 + text_length).view(batch_size, 1, 1).expand(-1, -1, self.hidden_size)
        hidden_states.scatter_(1, eos_idx, eos_embed)

        # Scatter bos to position 9 + text_length (CORRECTED from 10!)
        bos_idx = (9 + text_length).view(batch_size, 1, 1).expand(-1, -1, self.hidden_size)
        hidden_states.scatter_(1, bos_idx, bos_embed)

        # === Create attention mask ===
        # actual_len = 8 + text_length + 2 = text_length + 10 (CORRECTED from 11!)
        ...
        # Stack KV cache: [56, B, num_kv_heads, seq_len, head_dim]
        kv_list = []
...
def main():
    print("=" * 60)
    print("Qwen3-TTS LM Prefill V8 - CORRECTED Sequence")
    print("=" * 60)
    ...
    print(f"Expected sequence length: {text_len + 10}")
    ...
    codes_list, _ = result
    official_first = codes_list[0][0, 0].item()
    print(f"Official first token: {official_first}")

    # Logits correlation
    with torch.no_grad():
        result2 = tts_model.model.generate(
            input_ids=[full_input_ids],
            languages=['english'],
            voice_clone_prompt=voice_clone_prompt,
            non_streaming_mode=True,
    ...
    if first_token == official_first:
        print("MATCH! First tokens are identical.")
    else:
        print(f"MISMATCH: wrapper={first_token}, official={official_first}")
    ...
    mlmodel.save("qwen3_tts_lm_prefill_v8.mlpackage")
    print("Saved: qwen3_tts_lm_prefill_v8.mlpackage")

    import subprocess
    result = subprocess.run(['du', '-sh', 'qwen3_tts_lm_prefill_v8.mlpackage'], capture_output=True, text=True)
