# Test if code_predictor has KV cache issues
import torch
import numpy as np
import random
...
# Run V9 prefill FIRST (before official)
print("\n=== Running V9 prefill FIRST ===")
...
print(f"V9 first token: {first_token}")

# V3 decode step 0 - run BEFORE official
print("\n=== Running V3 code_predictor BEFORE official ===")
with torch.no_grad():
    ...
    print(f"V3 predictor input: mean={v3_predictor_input.mean().item():.6f}")
    ...
    v3_tokens_before = v3_result.sequences[0].tolist()

print(f"V3 predictor tokens (BEFORE official): {v3_tokens_before}")
...
# Capture official code_predictor inputs
official_predictor_inputs = []
original_predictor_generate = talker.code_predictor.generate

def capture_predictor_generate(*args, **kwargs):
    inputs_embeds = kwargs.get('inputs_embeds')
    if inputs_embeds is not None:
        official_predictor_inputs.append({
            'inputs_embeds': inputs_embeds.clone().detach(),
        })
    result = original_predictor_generate(*args, **kwargs)
    if len(official_predictor_inputs) > 0 and 'sequences' not in official_predictor_inputs[-1]:
        official_predictor_inputs[-1]['sequences'] = result.sequences.clone().detach()
    return result
...
# Compare
if len(official_predictor_inputs) > 0:
    off = official_predictor_inputs[0]
    off_tokens = off['sequences'][0].tolist()
    print(f"\nOfficial predictor tokens (step 0): {off_tokens}")
    print(f"V3 predictor tokens (BEFORE official): {v3_tokens_before}")
    print(f"Match: {off_tokens == v3_tokens_before}")

    # Compare inputs
    off_ie = off['inputs_embeds']
    print(f"\nOfficial input: mean={off_ie.mean().item():.6f}")
    print(f"V3 input:       mean={v3_predictor_input.mean().item():.6f}")
    input_diff = (off_ie - v3_predictor_input).abs().max().item()
    print(f"Input max diff: {input_diff:.6f}")

# Run V3 code_predictor AFTER official
print("\n=== Running V3 code_predictor AFTER official ===")
with torch.no_grad():
    v3_result_after = talker.code_predictor.generate(
        inputs_embeds=v3_predictor_input,
        max_new_tokens=config.num_code_groups - 1,
        do_sample=False,
        return_dict_in_generate=True,
    )
    v3_tokens_after = v3_result_after.sequences[0].tolist()

print(f"V3 predictor tokens (AFTER official): {v3_tokens_after}")
print(f"Same as BEFORE: {v3_tokens_after == v3_tokens_before}")
