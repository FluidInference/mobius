# Debug V9's full forward vs official
import torch
import numpy as np
...
print(f"V9 hidden shape: {v9_hidden.shape}, actual_len: {actual_len.item()}")
...
# Run V9 through all layers
print("\n=== Running V9 through all layers ===")
with torch.no_grad():
    ...
    # Final norm
    hidden_states = wrapper.norm(hidden_states)

    # Get logits from bos position
    bos_position = (actual_len - 1).view(batch_size, 1, 1).expand(-1, -1, config.hidden_size)
    last_hidden = torch.gather(hidden_states, 1, bos_position)
    v9_logits = wrapper.codec_head(last_hidden).squeeze(1)

    print(f"V9 final hidden at position 24: {hidden_states[0, 24, :5].tolist()}")
    print(f"V9 last_hidden (gathered): {last_hidden[0, 0, :5].tolist()}")
...
with torch.no_grad():
    ...
    print(f"Official final hidden at position 24: {official_hidden[0, 24, :5].tolist()}")

    official_logits = talker.codec_head(official_hidden[:, -1:, :]).squeeze(1)

# Compare
print("\n=== Comparison ===")
v9_final_pos24 = hidden_states[0, 24, :]
official_final_pos24 = official_hidden[0, 24, :]
diff = (v9_final_pos24 - official_final_pos24).abs().max().item()
print(f"Max diff at position 24 after all layers: {diff}")

# Compare logits
logits_diff = (v9_logits - official_logits).abs().max().item()
print(f"Max logits diff: {logits_diff}")
...
# Check if the problem is in gather vs direct indexing
print("\n=== Checking gather vs direct indexing ===")
with torch.no_grad():
    # Direct indexing at position 24
    direct_hidden = hidden_states[0, 24, :].unsqueeze(0).unsqueeze(0)
    direct_logits = wrapper.codec_head(direct_hidden).squeeze(1)

    direct_logits_np = direct_logits.numpy().copy()
    direct_logits_np[0, suppress_mask] = -float('inf')
    direct_token = int(np.argmax(direct_logits_np))

    print(f"V9 with direct indexing token: {direct_token}")
    print(f"Gathered position: {(actual_len - 1).item()}")
