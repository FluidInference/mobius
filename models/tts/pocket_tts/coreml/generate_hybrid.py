"""Generate audio using PyTorch FlowLM + CoreML Mimi decoder.

This is the practical hybrid approach:
- FlowLM (backbone + flow decoding): PyTorch
- Mimi decoder: CoreML

The FlowLM backbone was converted to CoreML but the flow decoding (LSD steps)
requires complex iterative logic that's harder to convert. For practical use,
keeping FlowLM in PyTorch and only using CoreML for the decoder is a good tradeoff.
"""
import torch
import numpy as np
...
def generate_hybrid(text: str, voice: str = "alba", output_path: str = "hybrid_output.wav"):
    """Generate audio using PyTorch FlowLM + CoreML Mimi decoder."""
    print(f"Text: '{text}'")
    print(f"Voice: {voice}")
    ...
    for step in range(max_gen_len):
        # Generate latent using PyTorch FlowLM (backbone + flow decoding)
        with torch.no_grad():
...
if __name__ == "__main__":
    generate_hybrid(
        "Hello, this is generated using a hybrid approach with PyTorch FlowLM and CoreML Mimi decoder.",
        voice="alba",
        output_path="hybrid_generated.wav"
    )
