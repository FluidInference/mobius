"""Parity test (v1.1-zh): PyTorch Kokoro reference vs 7-stage CoreML chain.
...
def pytorch_reference(phonemes, voice="zf_001", lang="z", repo_id=DEFAULT_REPO_ID):
    ...
    """
    model = KModel(repo_id=repo_id); model.eval()
    pipe = KPipeline(lang_code=lang, repo_id=repo_id, model=model)
...
def main():
    ...
    p.add_argument("--text", default=None,
                   help=f"Mandarin text to synthesize on both sides (default: {DEFAULT_TEXT!r})")
    p.add_argument("--phonemes", default=None,
                   help="Pre-computed Bopomofo+digit-tone phonemes (skips G2P, overrides --text)")
    p.add_argument("--voice", default="zf_001")
    ...
    if args.phonemes:
        ...
        # Run misaki[zh] G2P via Kokoro pipeline (one-time, before pytorch_reference reloads model)
        text = args.text or DEFAULT_TEXT
        print(f"[g2p] Phonemizing {text!r} via misaki[zh]...", file=sys.stderr)
        m_g = KModel(repo_id=args.repo_id); m_g.eval()
        pipe_g = KPipeline(lang_code=args.lang, repo_id=args.repo_id, model=m_g)
        phonemes = ""
        for _gs, ps, _tks in pipe_g(text, voice=args.voice):
            phonemes = ps
            break
        if not phonemes:
            p.error(f"misaki[zh] G2P produced empty phonemes for {text!r}")
        print(f"      phonemes ({len(phonemes)}): {phonemes!r}", file=sys.stderr)
        del m_g, pipe_g  # free for pytorch_reference reload

    print(f"[ref] PyTorch teacher synthesizing {len(phonemes)} phonemes...", file=sys.stderr)
    ref, artifacts = pytorch_reference(phonemes, voice=args.voice, lang=args.lang, repo_id=args.repo_id)
