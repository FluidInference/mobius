"""Standalone end-to-end CoreML inference for Kokoro-82M-v1.1-zh (Mandarin).

Loads the 7 .mlpackage files produced by convert-coreml.py, runs G2P (or
takes pre-computed Bopomofo+digit-tone phonemes), and writes a 24 kHz mono WAV.

Usage:
    uv run python inference.py \
        --models-dir build/kokoro-v1.1-zh \
        --text "你好世界" \
        --voice zf_001 \
        --output /tmp/out.wav

    # Skip G2P with pre-computed phonemes (matches iOS app flow):
    uv run python inference.py --models-dir build/kokoro-v1.1-zh \
        --phonemes "ㄋㄧˇㄏㄠˇ" --voice zf_001 --output /tmp/out.wav
...
def main():
    ...
    p.add_argument("--voice", default="zf_001", help="Voice id (default: zf_001 — Mandarin female)")
    p.add_argument("--lang", default="z", help="Kokoro lang code for G2P (default: z = Mandarin)")
    p.add_argument("--repo-id", default="hexgrad/Kokoro-82M-v1.1-zh",
                   help="HF repo id for PyTorch fallback (default: hexgrad/Kokoro-82M-v1.1-zh)")
    ...
    pt_model = None
    if needs_pipe:
        print(f"[setup] Loading Kokoro PyTorch ({args.repo_id}) for G2P / vocab fallback...", file=sys.stderr)
        pt_model = KModel(repo_id=args.repo_id); pt_model.eval()
        pipe = KPipeline(lang_code=args.lang, repo_id=args.repo_id, model=pt_model)
